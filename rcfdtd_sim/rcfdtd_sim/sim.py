...
class TensorMat:
    r"""
    The TensorMat class is used to represent a material present in a simulation via a tensor with a time dimension rather than opacity.

    :param dn: The temporal step size
    :param ilen: The number of spatial indicies in the simulation
    :param nlen: The number of temporal indicies in the simulation
    :param timebounds: A tuple `(timeStart, timeEnd)` of the time indicies between which the material exists in the simulation where `timeStart` is inclusive and `timeEnd` is exclusive. Multiple materials with carefully chosen time bounds can be used to change material properties in time.
    :param mati0: The starting index of the material in space.
    :param matn0: The starting index of the material in time.
    :param epsiloninf: :math:`\epsilon_\infty` inside the material
    :param mata1: A tensor representing :math:`A_1`. For a 3-dimensional tensor axis=0 represents the :math:`n` the temporal index, axis=1 represents the :math:`i` th spatial index, and axis=2 represents the :math:`j` th oscillator. For a 2-dimensional tensor axis=0 represents the :math:`n` th temporal index, and axis=1 represents the :math:`i` th spatial index. For a 1-dimensional tensor axis=0 represents the :math:`i` th spatial index, the material is assumed to exist for all time.
    :param mata2: A tensor representing :math:`A_2` where axis=0 represents the :math:`n` the temporal index, axis=1 represents the :math:`i` th spatial index, and axis=2 represents the :math:`j` th oscillator
    :param matg: A tensor representing :math:`\gamma` where axis=0 represents the :math:`n` the temporal index, axis=1 represents the :math:`i` th spatial index, and axis=2 represents the :math:`j` th oscillator
    :param matb: A tensor representing :math:`\beta` where axis=0 represents the :math:`n` the temporal index, axis=1 represents the :math:`i` th spatial index, and axis=2 represents the :math:`j` th oscillator
    :param tda1: Allows for time dependance of :math:`A_1`. `tda1` should be 1D Numpy array of length `nlen`. At each time step the value of `mata1` at all spatial indicies is multiplied by `tda1`.
    :param tda2: Allows for time dependance of :math:`A_2`. `tda2` should be 1D Numpy array of length `nlen`. At each time step the value of `mata2` at all spatial indicies is multiplied by `tda2`.
    :param storelocs: A list of locations to save chi at during each step in time, indexed from 0 to the material length
    :param dtype: The data type to store the field values in
    """

    def __init__(self, dn, ilen, nlen, mati0, matn0, epsiloninf, mata1, mata2, matg, matb, storelocs=[], dtype=np.complex64):
        # -------------
        # INITIAL SETUP
        # -------------
        # Check for error
        if np.shape(mata1) != np.shape(mata2) or np.shape(mata1) != np.shape(matg) or np.shape(mata1) != np.shape(matb):
            raise ValueError("The dimensions of mata1, mata2, matg, and matb should be the same")
        elif len(np.shape(mata1)) > 3:
            raise ValueError("The number of dimensions of mata1, mata2, matg, and matb cannot exceed 3")
        # Save arguments
        self._dn = dn
        self._ilen = ilen
        self._nlen = nlen
        ...
        self._matn0 = matn0
        self._dtype = dtype
        # Reshape material parameters as appropriate
        if len(np.shape(mata1)) == 3:
            # No reshaping required, simply extract tensor dimensions
            self._matnlen = np.shape(mata1)[0]
            self._matilen = np.shape(mata1)[1]
            self._jlen = np.shape(mata1)[2]
        elif len(np.shape(mata1)) == 2:
            # Extract tensor dimensions
            self._matnlen = np.shape(mata1)[0]
            self._matilen = np.shape(mata1)[1]
            self._jlen = 1
            # Reshape the arrays so that they can be indexed correctly later
            self._mata1 = np.reshape(mata1, (self._matnlen, self._matilen, self._jlen))
            self._mata2 = np.reshape(mata2, (self._matnlen, self._matilen, self._jlen))
            self._matg = np.reshape(matg, (self._matnlen, self._matilen, self._jlen))
            self._matb = np.reshape(matb, (self._matnlen, self._matilen, self._jlen))
        else:
            # Extract tensor dimensions
            self._matilen = np.shape(mata1)[0]
            self._matnlen = self._nlen
            self._jlen = 1
            # Tile the array so that it is defined for each time value
            self._mata1 = np.tile(mata1, (self._matnlen, 1, 1))
            self._mata2 = np.tile(mata2, (self._matnlen, 1, 1))
            self._matg = np.tile(matg, (self._matnlen, 1, 1))
            self._matb = np.tile(matb, (self._matnlen, 1, 1))
        # Check for error
        if self._mati0 < 0 or self._mati0 + self._matilen > self._ilen:
            raise ValueError("Material cannot start at i=" + str(self._mati0) + " and end at i=" + str(self._mati0 + self._matilen) + " as this exceeds the dimensions of the simulation.")
        elif self._matn0 < 0 or self._matn0 + self._matnlen > self._nlen:
            raise ValueError("Material cannot start at n=" + str(self._matn0) + " and end at n=" + str(self._matn0 + self._matnlen) + " as this exceeds the dimensions of the simulation.")
        # Epsilon_infinity is equal to one in vacuum, so only set self._epsiloninf equal to epsiloninf in the material
        epsiloninf_repeat = np.repeat(epsiloninf, self._matilen)
        self._epsiloninf = np.pad(epsiloninf_repeat, (self._mati0, self._ilen - (self._mati0 + self._matilen)), 'constant', constant_values=1)
        # --------------
        # MATERIAL SETUP
        # --------------
        # Calculate susceptability beta and gamma sums and exponents
        ...
        self._exp_2 = np.exp(np.multiply(min_b_min_g, self._dn))
        # Calculate initial susceptability values
        self._chi0_1 = np.zeros((self._jlen, self._matilen), dtype=self._dtype) # Set chi0_1=0 initially
        self._chi0_2 = np.zeros((self._jlen, self._matilen), dtype=self._dtype) # Set chi0_2=0 initially
        for j in range(self._jlen):
            for mi in range(self._matilen):
                if np.abs(b_min_g[j, mi]) < 1e-5:
                    # beta-gamma is small, avoid divide by zero error
                    self._chi0_1[j, mi] = self._mata1[j, mi]*self._dn
                    self._chi0_2[j, mi] = np.multiply(np.divide(self._mata2[j, mi], min_b_min_g[j, mi]), np.subtract(self._exp_2[j, mi], 1))
        ...
        self._dchi0_2 = np.multiply(self._chi0_2, np.subtract(1, self._exp_2))
        # Initialize psi values to zero
        self._psi_1 = np.zeros((self._jlen, self._matilen), dtype=self._dtype)
        self._psi_2 = np.zeros((self._jlen, self._matilen), dtype=self._dtype)
        # Calculate chi0
        chi0_j = np.add(self._chi0_1, self._chi0_2)
        chi0 = np.sum(chi0_j, axis=2) # Sum over the oscillators (axis=2)
        # Pad chi0 so that it spans the length of the simulation
        chi0_padded = np.pad(chi0, (self._mati0, self._ilen - (self._mati0 + self._matilen)), 'constant')
        self._chi0 = chi0_padded
        # -------------------
        # STORED VALUES SETUP
        # -------------------
    ...
    def __eq__(self, other):
        ...
        """
        if isinstance(other, Mat):
            """dn, ilen, nlen, mati0, epsiloninf, mata1, mata2, matg, matb"""
            dn_eq = (self._dn == other._dn)
            ilen_eq = (self._ilen == other._ilen)
            nlen_eq = (self._nlen == other._nlen)
            mat0_eq = (self._mati0 == other._mati0)
            epsiloninf_eq = (self._epsiloninf == other._epsiloninf)
            mata1_eq = np.array_equal(self._mata1, other._mata1)
            mata2_eq = np.array_equal(self._mata2, other._mata2)
            matg_eq = np.array_equal(self._matg, other._matg)
    ...
    def get_pos(self):
        ...
        :return: `(arr, time0, time1)`
        """
        return (np.pad(np.repeat(1, self._matilen), (self._mati0, self._ilen - (self._mati0 + self._matilen)), 'constant'), self._time0, self._time1)
    ...
    def _compute_psi(self, n):
        ...
        psi = np.sum(psi_j, axis=0)
        # Pad the psi array so that it spans the length of the simulation
        psi_padded = np.pad(psi, (self._mati0, self._ilen - (self._mati0 + self._matilen)), 'constant')
        # Return the real part as specified in Beard
    ...
    def _update_psi(self, efield):
        ...
        """
        # Copy the efield so that instead of being a vector it is a matrix composed of horizontal efield vectors
        e = np.tile(efield[self._mati0:self._mati0 + self._matilen], (self._jlen, 1))
        # Calculate first term
        t1_1 = np.multiply(e, self._dchi0_1)
        t1_2 = np.multiply(e, self._dchi0_2)
        # Calculate second term
