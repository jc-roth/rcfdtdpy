...
eps_mat = 8;                                       # high frequency dielectric constant of material

L = 300;                                           # (um) thickness of sample
nu_j = 0.1;                                           # (THz) resonant frequency of oscillator
...
dz = 0.3;                                             # (um) size discretization
dt = R*dz/c0;                                           # (ps) timestep

t = np.arange(-120, 6000,dt);                                # (ps) simulation time window 
z_pos = np.arange(c0/R*-120,c0/R*480,dz);                   # (um) spatial box size

print('len(t)=%i' % len(t))
print('len(z)=%i' % len(z_pos))

# N = np.size(t);
N = 2**16;

z1 = np.min(np.abs(z_pos));                             # finding zero for setting the position of the sample
z_int_1 = np.argmin(np.abs(z_pos))
...
f1_ax1.plot(t,Sig_trc)

f1_ax1.plot(t,Ref_trc)
plt.show()
...
#freq = fftaxis(N,1/dt);
#omega = 2*pi*ifftshift(freq);

#sig_spec = fft(Sig_trc-mean(Sig_trc),N);
#ref_spec = fft(Ref_trc-mean(Ref_trc),N);

#t_spec = sig_spec./ref_spec;
#phi = -phase(t_spec);
#Abs_spec = abs(t_spec);

#n = c./(omega'*L).*phi + 1;

#k = -c./(omega'*L).*log(Abs_spec.*(n+1).^2./(4*n));
