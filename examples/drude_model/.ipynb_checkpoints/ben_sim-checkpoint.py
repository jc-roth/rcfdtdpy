...
eps_mat = 100;                                       # high frequency dielectric constant of material

L = 0.012;                                           # (um) thickness of sample
nu_j = 0;                                           # (THz) resonant frequency of oscillator
gamma = 500;                                       # (THz) linewidth
gamma_j = gamma * 2 * np.pi;

beta_j = np.sqrt((gamma_j)**2 - (2*np.pi*nu_j)**2);      # (THz) effective oscillation frequency
A = 5e10;                                            # (a.u.) oscillator amplitude
...
t = np.arange(-2,100,dt);                                # (ps) simulation time window 
...
np.savez('ben_sim.npz', t=t, z_pos=z_pos, chi_t=chi_t, Sig_trc=Sig_trc, Ref_trc=Ref_trc)